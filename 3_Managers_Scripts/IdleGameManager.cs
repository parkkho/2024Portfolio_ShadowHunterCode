...
public class IdleGameManager : MonoBehaviour // 방치형 게임 전체적인 관리
{
    ...
    //// 생명력 흡수 및 변신 에너지 획득 연출
    //public void EarnBloodEnergy(Vector2 pos, float hp)
    //{
    //    GameObject bloodObj;

    //    if (bloodPool.Count > 0)
    //    {
    //        bloodObj = bloodPool[0];

    //        bloodPool.RemoveAt(0);
    //    }
    //    else
    //    {
    //        bloodObj = Instantiate(bloodTemplate, transform);
    //    }
    ...
    //    StartCoroutine(MoveBloodEffect(bloodObj, hp));

    //    //  bloodObj.transform.DOMove(player.transform.position, 0.5f).OnComplete(() => { bloodObj.SetActive(false); bloodPool.Add(bloodObj); });
    //}

    //IEnumerator MoveBloodEffect(GameObject bloodObj, float hp)
    //{
    ...
    //    Vector3 sPos = bloodObj.transform.position;
    ...
    //        bloodObj.transform.position = Vector3.Lerp(sPos, player.transform.position, time / moveTime);

    //        yield return null;
    //    }

    //    player.playerAtkHandler.MonsterLifeSteal(hp);
    //    player.EarnTransformEnergy(0.01f);

    //    bloodObj.SetActive(false);
    //    bloodPool.Add(bloodObj);
    ...
    //public void ControlBgVolumeColor(Color color)
    //{
    //    Vignette vignette;

    //    if (bgGlobalVolume.profile.TryGet(out vignette))
    //    {
